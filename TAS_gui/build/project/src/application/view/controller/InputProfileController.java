...
public class InputProfileController implements Initializable {
	...
	private void initializeReqTable(){
		...
		nameColumn.prefWidthProperty().bind(valueTableView.widthProperty().divide(3));
		...
		typeColumn.prefWidthProperty().bind(valueTableView.widthProperty().divide(3));
		...
		dataColumn.prefWidthProperty().bind(valueTableView.widthProperty().divide(3));
		...
		reqTableView.getColumns().addAll(nameColumn,typeColumn,dataColumn);
	}
			...
			reqListView.getSelectionModel().selectedItemProperty().addListener(
	                new ChangeListener<String>() {
	                    public void changed(ObservableValue<? extends String> val, 
	                    	...
	                    	reqData.add(new ValueEntry(currentReq.getName(),currentReq.getType(), 
	                    				currentReq.getData(), null, null));
	                }
	            });
